...
namespace NexusProject.API.Controllers
{
    ...
    [Route("/api/ActivityCollection2")]
    public class ActivityCollectionController2 : ControllerBase
    {

        private readonly DataContext _context;

        //Constructor
        public ActivityCollectionController2(DataContext context)
        {
            _context = context;
        }

        [HttpGet]
        public async Task<IActionResult> GetAsync([FromQuery] PaginationDTO pagination, [FromQuery] int Youngid)
        {
            var queryable = _context.ActivityColections
                .Where(ac => ac.Youngid == Youngid)  
                .AsQueryable();
        ...
        [HttpGet("totalPages")]
        public async Task<ActionResult> GetPages([FromQuery] PaginationDTO pagination, [FromQuery] int Youngid)
        {
            var queryable = _context.ActivityColections
                .Where(ac => ac.Youngid == Youngid)  
                .AsQueryable();
