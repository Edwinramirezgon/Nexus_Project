        ...
        public class AccountsController : ControllerBase
        {
            ...
            public async Task<ActionResult> RecoverPassword([FromBody] EmailDto model)
            {
                ...
                var response = _mailHelper.SendMail(user.FullName, user.Email!,
                    $"Nexus Project - Password Recovery",
                    $"<h1>Nexus Project - Password Recovery</h1>" +
            ...
            public async Task<ActionResult> CreateUser([FromBody] UserDTO model)
            {
                ...
                if (user.UserType != 0)
                {
                    ...
                    try
                    {
                        if (result.Succeeded)
                        {
                            ...
                            var response = _mailHelper.SendMail(user.FullName, "edwinramirezgon@gmail.com",
                                $"Nexus Project - Account Confirmation",
                                $"<h1>Nexus Project - Account Confirmation</h1>" +
                                $"<p>To enable the user, please click 'Activate'':</p>" +
                                $"<b><a href ={tokenLink}>Activate</a></b>");

                            var response2 = _mailHelper.SendMail(user.FullName, user.Email!,
                                $"Nexus Project - Welcome",
                                $"<h1>Thank you for Register just wait for activation of your account</h1>");
                ...
                {
                    return BadRequest("You must Select a User Type");
                }
            }


            [HttpPost("CreateAdmin")]
            public async Task<ActionResult> PostAsync(Admin user)
            {
                _context.Add(user);
            ...
            [HttpPost("CreateYoung")]
            public async Task<ActionResult> PostAsync(Young user)
            {
                _context.Add(user);
                await _context.SaveChangesAsync();
                return Ok(user);
            ...
            public async Task<ActionResult> Login([FromBody] LoginDTO model)
            {
                ...
                {
                    return BadRequest(" The username has not been enabled, you must wait an admin to enable this user.<br><br><br><a class=\"btn btn-outline-info\" href=\"/ResendToken\">Resend activation requestl</a>");
                }
